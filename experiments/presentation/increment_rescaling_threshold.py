...
from simulation.estimators import truncation_threshold
...
def plot_increment_rescaling_with_threshold(
    results, title="Increment rescaling as n grows (with truncation threshold)",
    scale=3.0, tau=0.475,
):
    n_cols = len(results)
    ...
    for col, r in enumerate(results):
        ...
        u_n = truncation_threshold(r["n"], scale, tau)

        inc_max = np.nanmax(np.abs(r["increments"]))
        top_lim = 1.1 * max(inc_max, u_n)
        top.plot(r["t"], r["increments"], linewidth=0.7, color="tab:blue")
        reference_line(top, u_n, color="crimson")
        reference_line(top, -u_n, color="crimson")
        top.set_ylim(-top_lim, top_lim)
        top.set_title(f"n = {r['n']}")
        top.set_xlabel("t")
        top.text(
            0.02, 0.95, f"$u_n$={u_n:.3f}",
            transform=top.transAxes, ha="left", va="top", fontsize=8, color="crimson",
        )
...
def main():
    ...
    parser.add_argument("--tau", type=float, default=0.475)
    parser.add_argument("--out", type=Path, default=Path("figures/presentation"))
    args = parser.parse_args()
    args.out.mkdir(exist_ok=True, parents=True)
    stem = ("increment_rescaling_threshold_jumps" if args.with_jumps
            else "increment_rescaling_threshold")
    ...
    results = increment_rescaling_path(
        args.n_values, heston, jumps=jumps, scale=args.scale, tau=args.tau, seed=args.seed
    )
    ...
    title = "Increment rescaling as n grows (with truncation threshold)" + (
        " (with jumps)" if args.with_jumps else ""
    )
    plot_increment_rescaling_with_threshold(
        results, title, scale=args.scale, tau=args.tau,
    ).savefig(figure_path)
    write_report(
